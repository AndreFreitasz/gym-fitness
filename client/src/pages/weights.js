...
const Weights = () => {
  ...
  const searchUserData = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:3001/searchUserData", {
        params: { idUser },
      });
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3001/postWeightRecord",
        { ...values, idUser },
      );
  ...
  useEffect(() => {
    const filtered = exercises.filter((exercise) => {
      return (
        (filter.exercise === "" ||
          exercise.name_exercise === filter.exercise) &&
        (filter.date === "" || exercise.record_weight_date === filter.date) &&
        (filter.muscleGroup === "" ||
          exercise.muscle_group_name === filter.muscleGroup)
      );
    });
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.delete(
            "http://localhost:3001/deleteWeightRecord",
            { data: { id } },
          );
          if (response.status === 200) {
  ...
  const uniqueExercises = [
    ...new Set(exercises.map((exercise) => exercise.name_exercise)),
  ];
  const uniqueDates = [
    ...new Set(exercises.map((exercise) => exercise.record_weight_date)),
  ];
