...
const ModalViewWeights = ({ isOpen, onRequestClose, title, rowData }) => {
    ...
    const maxWeight = weightsData.length > 0 ? Math.max(...weightsData.map(weight => parseWeight(weight.record_weight))) : null;
    ...
    const searchDatesWeightsByExercise = async () => {
        ...
        try {
            const response = await axios.get('http://localhost:3001/searchDatesWeightsByExercise', { params: { idUser, idExercise } });
                                ...
                                {weightsData.length > 0 ? (
                                    <>
                                        <div className="flex justify-around text-white font-semibold mb-2 text-2xl">
                                            <span className="inline-block border-b-4 rounded-b-sm border-red-500">Data</span>
                                            <span className="inline-block border-b-4 rounded-b-sm border-red-500">Peso</span>
                                        </div>
                                                ...
                                                {weightsData.map((weight, index) => (
                                                    <span 
                                                        ...
                                                        className={`text-white mb-5 text-lg px-2 rounded-md 
                                                            ${
                                                                parseWeight(weight.record_weight) === maxWeight 
                                                                ? 'bg-green-500' 
                                                                : 'bg-red-500'
                                                            }`}
                                                    >
